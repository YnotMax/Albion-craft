...
export const Transport: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      for (const chunk of chunks) {
        const url = `https://${serverPrefix}.albion-online-data.com/api/v2/stats/prices/${chunk.join(',')}?locations=${buyCity},${sellCity}&qualities=${qualities}`;
  ...
  return (
    <div className="space-y-6 font-['Inter'] pb-10">
      ...
      {/* Dicas de Transporte */}
      <div className="bg-amber-900/20 border border-amber-500/30 rounded-xl p-4 flex gap-4">
        <AlertTriangle className="text-amber-500 mt-1 shrink-0" />
        <div className="text-sm text-amber-100/80">
          <strong className="text-amber-500 block mb-1">Dicas de Fuga (Red Zones):</strong>
          Utilize equipamento 100% focado em sobrevivência: <span className="text-amber-200">Sangra-sugas + Chamado da Névoa</span>, 
          Armadura de Bravura, Botas e Gorro de Minerador. Consumíveis recomendados: Poção de Invisibilidade e Torta de Porco.
          Procure rotas pelas <span className="text-amber-200">Estradas Avalonianas</span> ou aproveite as invasões de Facção (Assalto de Bandidos).
        </div>
      </div>
